...
3. Plugin versions follow SemVer: MAJOR for removed/renamed skills or breaking behavior, MINOR for new skills or new user-facing capabilities, PATCH for metadata/docs/example fixes that do not change behavior
4. When bumping plugin version: update the Claude plugin manifest, Claude marketplace `plugins[0].version`, and Codex plugin manifest
5. CHANGELOG.md tracks plugin changes only — CLI versions follow upstream release tags
