...
- A patch file at a known path (convention: `build/patch.json`)
...
```bash
grepr pipeline:edit --job-id <JOB_ID> --patch build/patch.json -o build/plan.json
```
...
```bash
jq -r '.classification' build/plan.json
```

If the classification is `touches-sink` or `mixed`, **stop**. The draft
harness cannot validate sink delivery. Surface:

> "This patch is classified `<classification>`. Sink delivery isn't
> verifiable through the draft harness — there's no readback from the
> destination vendor. Either split the patch (apply transform/source ops
> separately from sink ops) or accept the risk and apply the sink portion
> with operator review."

For `transform-only` and `touches-source` classifications, continue.

## Step 2: Show the User the Diff

```bash
grepr pipeline:plan --job-id <JOB_ID> --patch build/patch.json
```
...
```bash
grepr pipeline:draft build/plan.json > build/draft-output.ndjson
```
...
echo '{"operations": []}' > build/no-op-patch.json
grepr pipeline:edit --job-id <JOB_ID> --patch build/no-op-patch.json -o build/baseline-plan.json
grepr pipeline:draft build/baseline-plan.json > build/baseline-output.ndjson
```
...
```bash
grepr pipeline:apply build/plan.json
```
...
- `build/patch.json` — input from the previous skill
- `build/plan.json` — generated by step 1, consumed by step 6
- `build/draft-output.ndjson` — streamed draft results
- `build/baseline-output.ndjson` (optional) — baseline for comparison
- `build/no-op-patch.json`, `build/baseline-plan.json` (optional) — baseline scaffolding

All under `build/` per repo convention. Don't use `/tmp/`.
