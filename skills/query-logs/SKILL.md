...
- Use `--format raw` to get JSON output for easier parsing, or `--format table` for human-readable output.
...
- When displaying results for a user, use `--format table` for human-readable output. Use `--format raw` for JSON output that is easier to parse programmatically and show the output to the user.
...
grepr query --dataset-id ds-123456 --query "state change @user.id:jaja" --start "2024-08-20T00:00:00Z" --end "2024-08-21T00:00:00Z" --limit 50 --format raw
```
